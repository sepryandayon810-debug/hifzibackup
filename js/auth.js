...
// ==========================================
// KASIR SHIFT LOCK SYSTEM
// Mencegah akses menu jika belum buka kasir/shift
// ==========================================

const KasirLock = {
  STORAGE_KEY: 'webpos_kasir_shift',
  
  isOpen: function() {
    const data = Utils.getStorage(this.STORAGE_KEY);
  ...
  open: function(userId, modalAwal) {
    const shiftData = {
      status: 'open',
      openedAt: Date.now(),
      openedBy: userId || 'unknown',
      modalAwal: modalAwal || 0,
      device: navigator.userAgent
    };
    Utils.setStorage(this.STORAGE_KEY, JSON.stringify(shiftData));
    Utils.showToast('Kasir berhasil dibuka! Semua menu aktif.', 'success');
    this._removeOverlay();
    this._updateBadge(true);
  },
  
  close: function() {
    Utils.removeStorage(this.STORAGE_KEY);
    Utils.showToast('Kasir ditutup. Menu terkunci.', 'info');
    this._updateBadge(false);
    setTimeout(() => location.reload(), 500); // Refresh agar lock langsung aktif
  },
  
  requireOpen: function(e) {
    if (!this.isOpen()) {
      if (e) {
        e.preventDefault();
        e.stopPropagation();
      }
      Utils.showToast('Anda harus membuka kasir terlebih dahulu!', 'warning');
      setTimeout(() => {
        if (!window.location.href.includes('index.html')) {
          window.location.href = 'page-kasir.html';
        }
      }, 800);
      return false;
    }
  ...
  init: function() {
    // Halaman yang diizinkan meski belum buka kasir
    const file = location.pathname.split('/').pop() || 'index.html';
    const allowed = ['login.html', 'page-kasir.html', 'page-setting.html', ''];
  ...
  _blockMenus: function() {
    const allowed = ['login.html', 'page-kasir.html', 'page-setting.html'];
    document.querySelectorAll('a[href]').forEach(link => {
      const href = link.getAttribute('href') || '';
      const page = href.split('/').pop().split('?')[0];
      if (page && !allowed.includes(page) && !href.startsWith('#') && !href.startsWith('javascript')) {
        link.addEventListener('click', (e) => this.requireOpen(e));
      }
    });
  },
  
  _showOverlay: function() {
    if (document.getElementById('kasirLockOverlay')) return;
    const overlay = document.createElement('div');
    overlay.id = 'kasirLockOverlay';
    overlay.innerHTML = `
      <div style="position:fixed;inset:0;background:rgba(15,23,42,0.85);backdrop-filter:blur(4px);z-index:9997;display:flex;align-items:center;justify-content:center;padding:1rem;">
        <div style="background:var(--bg-card);padding:2rem;border-radius:1rem;text-align:center;max-width:380px;border:1px solid var(--border-color);">
          <div style="width:64px;height:64px;background:rgba(239,68,68,0.1);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;color:var(--danger);font-size:1.5rem;"><i class="fas fa-lock"></i></div>
          <h3 style="margin-bottom:0.5rem;color:var(--text-primary);">Kasir Belum Dibuka</h3>
          <p style="color:var(--text-muted);margin-bottom:1.5rem;font-size:0.9rem;">Anda harus membuka kasir terlebih dahulu untuk mengakses menu ini.</p>
          <a href="page-kasir.html" style="display:inline-flex;align-items:center;gap:0.5rem;padding:0.75rem 1.5rem;background:var(--primary);color:#fff;border-radius:0.75rem;text-decoration:none;font-weight:600;"><i class="fas fa-cash-register"></i> Buka Kasir Sekarang</a>
        </div>
      </div>
    `;
  ...
  _removeOverlay: function() {
    const el = document.getElementById('kasirLockOverlay');
    if (el) el.remove();
  },
  
  _updateBadge: function(isOpen) {
    let badge = document.getElementById('kasirShiftBadge');
    const target = document.querySelector('.header-actions');
    if (!target) return;
    if (!badge) {
      badge = document.createElement('div');
      badge.id = 'kasirShiftBadge';
      target.insertBefore(badge, target.firstChild);
    }
    badge.style.cssText = `padding:0.35rem 0.75rem;border-radius:9999px;font-size:0.75rem;font-weight:600;display:inline-flex;align-items:center;gap:0.35rem;background:${isOpen ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)'};color:${isOpen ? 'var(--success)' : 'var(--danger)'};`;
    badge.innerHTML = `<i class="fas fa-circle" style="font-size:0.5rem;"></i> ${isOpen ? 'Shift Aktif' : 'Shift Tutup'}`;
  }
};

// Auto-init setelah auth siap
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => KasirLock.init(), 150);
});
